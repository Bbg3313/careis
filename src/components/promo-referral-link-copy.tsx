...
export function PromoReferralLinkCopy({
  ...
  return (
    <div className={`flex min-w-0 flex-wrap items-center gap-2 ${compact ? "" : "flex-col items-stretch sm:flex-row sm:items-center"}`}>
      <span
        className={
          compact
            ? "min-w-0 max-w-[min(100%,280px)] truncate font-mono text-[11px] text-stone-600"
      ...
      <button
        ...
        onClick={() => void onCopy()}
        disabled={!fullUrl}
        className="shrink-0 rounded-full border border-stone-300 bg-white px-3 py-1 text-xs font-semibold text-stone-800 shadow-sm transition hover:bg-stone-50 disabled:cursor-not-allowed disabled:opacity-40"
      >
        {copyState === "done" ? "복사됨" : copyState === "err" ? "복사 실패" : "복사"}
      </button>
    </div>
  );
}
